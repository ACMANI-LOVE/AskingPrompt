...
import { DividerLine, DownloadingIcon, Layout, LText, MText, OrderChecker, PaperLayout, UploadingIcon } from '../atoms';
...
const MakingPanel = () => {
  ...
  const settingsList = [
    ...
    { label:"Details", settings:<DetailsSettings orderSelect={selection.orderSelect}/> },
  ...
  const [SettingsTab, selectSettingsTab] = useTabGroup({
    ...
    labelList: settingsList.map((item,idx)=><MText key={`setLbl${idx}`} text={`${zeroPads(idx+1)}:${item.label}`}/>),
  })
  ...
  return(<Layout center vertical>
    <Layout>
      ...
      <Layout size={8}>
        <LText bold text={"Fin. Set the Prompts"+` --- Selected:[${zeroPads(selection.orderSelect+1)}]`}/>
      </Layout>
    </Layout>
    <Layout>
      <Layout size={2}>
